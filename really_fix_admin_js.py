...
js_code = '''
  ...
  async function fetchSystemMetrics() {
    ...
    try {
      ...
      if (data.success) {
        renderCharts(data.metrics.reverse());
      }
    } catch(e) {
  ...
  function renderCharts(metrics) {
    ...
    createOrUpdateChart('tpsChart', [
      { label: 'Search TPS', data: metrics.map(m => m.tps_search), borderColor: '#059669', backgroundColor: '#05966933', fill: true },
      { label: 'Click TPS', data: metrics.map(m => m.tps_click), borderColor: '#D97706', backgroundColor: '#D9770633', fill: true }
    ]);
...
if 'function fetchSystemMetrics' not in content:
    # We'll replace '<script>\n    let isAuthed = false;' with our code + the original code
    content = content.replace(
        '<script>\n    let isAuthed = false;',
        js_code + '\n    let isAuthed = false;'
    )
    
    # We also need to call fetchSystemMetrics() when loadAdminData is called
    content = content.replace(
        'loadAdminData();',
        'loadAdminData();\n    fetchSystemMetrics();'
    )
